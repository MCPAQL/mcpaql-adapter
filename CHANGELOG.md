...
- Discord MCP-AQL operation layer (`src/plugins/transport/discord-operations.ts`) — first piece of the runnable adapter (#38, part of #52)
  ...
  - The listings run every name (conversation, server, channel, category, raw label) through the untrusted-content classifier, since other people choose those names: `flags`, `flagged_ids`, and `highest_severity` travel with each listing and `redact` is accepted, exactly as for `read_messages` (`classifyListingItems` in `discord-untrusted.ts`)
  - The runtime guard enforces what the effects mean, not only their primitives: navigation may push only `/channels/` paths, and the only events that may be constructed or dispatched (inline literal only) are `popstate` under navigation and `scroll` under scrolling
...
- Discord `read_messages` right after a navigation: the route changes and the message list mounts before the rows render (observed live through the runnable adapter), so the first extraction reported the channel "not in view" and the read stopped with a problem. After a navigation in the call (including the cursor fallback), the read now polls the small row-count probe for up to 3 s until a row is seen, then extracts once; a budget that runs out during that wait is a `time_budget` stop (with `problem` saying the channel was still rendering) rather than a problem; a channel that was already open waits for nothing and behaves as before. One shared `defaultSleep` replaces three inline copies (#38)
