...
function scanLoop(bodyLines: string[]): string[] {
  return [
    ...
    "  } catch (e) {",
    "    // Only an out-of-range index (-1719) means end of mailbox; any",
    "    // other error (Apple Event timeout, transient Mail failure) must",
    "    // propagate — treating it as completion would silently truncate",
    "    // the scan and permanently stop cache backfill paging.",
    "    if (e.errorNumber === -1719) {",
    "      done = true;",
    "      break;",
    "    }",
    "    throw e;",
