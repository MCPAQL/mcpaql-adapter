...
- Discord page-script registry and read-only scan (`src/plugins/transport/discord-scripts.ts`, `test/discord-read-only.test.ts`) — part of #44 for the read-only Discord adapter (#38)
  ...
  - No script may use an input, network, storage, identity, or injection primitive (focus, click, key events, `fetch`, cookies, storage, `eval`, script elements, Discord client internals); gated primitives (`location.assign`, `scrollTop`, `dispatchEvent`) are allowed only under a declared effect, a declared effect must actually be used, dispatched events must be `scroll` only, and navigation targets must be `/channels/` paths
